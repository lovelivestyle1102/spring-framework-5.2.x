...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
	...
	// 在 Spring 容器刷新之前也就是初始化之前注册的监听器
	/** Local listeners registered before refresh. */
	@Nullable
	private Set<ApplicationListener<?>> earlyApplicationListeners;

	// 在 applicationEventMulticaster 设置之前发布的事件 供后面事件回放所用
	...
	protected void publishEvent(Object event, @Nullable ResolvableType eventType) {
		...
		if (event instanceof ApplicationEvent) {
			...
			// 普通事件这里会 封装为 PayloadApplicationEvent 类型
		...
		// 换言之，当前应用上下文的事件会执行两次（自己发了一次，父应用上下文也发了一次）
		if (this.parent != null) {
	...
	protected ConfigurableListableBeanFactory obtainFreshBeanFactory() {
		// 刷新 Spring 应用上下文底层 BeanFactory
		refreshBeanFactory();
		// 返回 Spring 应用上下文底层的 BeanFactory
		return getBeanFactory();
	}
	...
	protected void prepareBeanFactory(ConfigurableListableBeanFactory beanFactory) {
		...
		// 3. 添加 PropertyEditorRegistrar 实现
		...
		// 5. 忽略 Aware 回调接口作为依赖注入接口（spring 不建议注入 Aware）
		// 我们之所以实现 Aware 接口是要获取上下文环境的对象，依赖注入连上下文环境都没有，注入进来没有意义
		...
		// 8. 注册 LoadTimeWeaverAwareProcessor 对象 （AOP 相关）
	...
	protected void initMessageSource() {
		...
		if (beanFactory.containsLocalBean(MESSAGE_SOURCE_BEAN_NAME)) {
			// 实际上这里是可以从外部配置一个 messageSource， 例如 Springboot 就会自己定义一个 MessageSource 并使用 @Bean的方式注入进来
			...
			// 如果不存在，这里定义了一个默认的空的 MessageSource （空的意思就是有这个对象，但是对象里面并没有国际化文案内容）
			// 这里 Spring 注释也说了是为了支持 getMessage 调用
			// Use empty MessageSource to be able to accept getMessage calls.
			DelegatingMessageSource dms = new DelegatingMessageSource();
			// 委托给父类实现 DelegatingMessageSource 是实现了 HierarchicalMessageSource 层次性接口的
	...
	protected void initLifecycleProcessor() {
		...
		if (beanFactory.containsLocalBean(LIFECYCLE_PROCESSOR_BEAN_NAME)) {
			...
			// 创建一个默认的 LifecycleProcessor 对象 DefaultLifecycleProcessor
			DefaultLifecycleProcessor defaultProcessor = new DefaultLifecycleProcessor();
			// 设置 LifecycleProcessor 的 beanFactory 属性
			defaultProcessor.setBeanFactory(beanFactory);
			this.lifecycleProcessor = defaultProcessor;
			// 把 LifecycleProcessor 注册到 beanFactory
	...
	protected void registerListeners() {
		 ...
		 *  所以会把事件先放到 earlyApplicationEvents 里
		 *  这里就是从 earlyApplicationEvents 取出来进行事件回放重新再放回到 ApplicationEventMulticaster 中去
		 **/
	...
	protected void finishBeanFactoryInitialization(ConfigurableListableBeanFactory beanFactory) {
		...
		// 把临时的 ClassLoader tempClassLoader 置为空 （常用在 AOP ）
	...
	protected void finishRefresh() {
		...
		// 上面初始化完成，这里调用 onRefresh() 方法
		// Propagate refresh to lifecycle processor first.
		getLifecycleProcessor().onRefresh();

		// Publish the final event.
		// 发布 Spring ContextRefreshedEvent (应用上下文刷新) 事件
